...
export default class RoomTableRow extends Component {
  ...
  approveRequest() {
    axios.put('http://localhost:4000/event/update/' + this.props.obj._id, {status: "Approved"})
      .then((res) => {
  ...
  rejectRequest() {
    axios.put('http://localhost:4000/event/update/' + this.props.obj._id, {status: "Rejected"})
    .then((res) => {
      console.log('User successfully updated')
    }).catch((error) => {
      console.log(error)
